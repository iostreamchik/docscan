...
**Kotlin:** 2.4.0 | **AGP:** 9.2.1 | **OpenCV:** 5.0.0.1 | **ONNX Runtime:** 1.29.0 | **CameraX:** 1.6.1 | **Koin:** 4.2.2
...
4. **CornerKeypointDetector** (LCNet ONNX) as second fallback — currently skipped (`skipKeypointDetector = true` in `runOnnxFallbacks()`)
5. **SegmentationDetector** (DeepLabV3 ONNX) as final fallback

Frames are scaled to 448px max dimension in `CameraViewModel` (`PROCESS_WIDTH`) before being passed to all detectors. ONNX models are stored in `assets/onnx/`.
